...
import { SeverityBadge, StatusBadge, VisibilityBadge } from './Badges'
import { ShieldCheck, ShieldAlert, CheckCircle2, Clock, ThumbsDown, ArrowUpRight } from 'lucide-react'
...
export default function IncidentDetail({ open, onClose, incident, onUpdated }) {
  ...
  return (
    <Modal
      open={open}
      onClose={onClose}
      title={`${incident.incidentId || 'INCIDENT'} — ${incident.cameraId}`}
      wide
    >
      <div className="space-y-5">
        ...
        {/* Evidence Snapshot */}
        {(incident.snapshot || incident.thumbnail) && (
          <div className="space-y-1.5">
            <img
              src={incident.snapshot || incident.thumbnail}
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 text-xs bg-white/[0.02] p-3.5 rounded-lg border border-ops-border">
          <Field label="Camera Outpost" value={incident.cameraId} />
          <Field
            label="Timestamp"
            value={incident.createdAt ? new Date(incident.createdAt).toLocaleTimeString() : '—'}
          />
          ...
          <Field label="Zone Classification" value={incident.zone || 'Normal Outpost Grid'} />
        ...
        {/* Operator Feedback / False Alarm Feedback */}
