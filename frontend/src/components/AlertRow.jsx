...
export default function AlertRow({ alert, onView, onAcknowledge, onResolve, canAct, selected, onToggleSelect }) {
  ...
  return (
    <tr className={`border-b border-ops-border hover:bg-white/[0.03] transition-colors ${isSelected ? 'bg-ops-accent/10' : ''}`}>
      ...
      <td className="px-4 py-3 text-xs font-mono text-ops-muted">{alert._id?.slice(-6)}</td>
      <td className="px-4 py-3 text-sm font-medium text-white">{alert.alertType || alert.eventType}</td>
      <td className="px-4 py-3"><SeverityBadge severity={alert.severity} /></td>
      <td className="px-4 py-3 text-sm text-ops-muted">{alert.cameraId}</td>
      <td className="px-4 py-3 text-xs text-ops-muted">{timeAgo(alert.timestamp)}</td>
      ...
      <td className="px-4 py-3">
        <div className="flex flex-col gap-0.5">
          <span className={`text-xs font-semibold px-2 py-0.5 rounded w-fit ${
            alert.status === 'NEW' || alert.status === 'DETECTED' ? 'bg-red-500/15 text-red-400' :
            alert.status === 'ACKNOWLEDGED' || alert.status === 'RESPONDING' ? 'bg-yellow-500/15 text-yellow-400' :
            'bg-green-500/15 text-green-400'
          }`}>{alert.status}</span>
          {alert.metrics?.resolutionTimeSeconds !== undefined && (
            <span className="text-[10px] text-ops-muted font-mono">
              Res: {alert.metrics.resolutionTimeSeconds}s
            </span>
          )}
        </div>
      </td>
      <td className="px-4 py-3">
        <div className="flex items-center gap-2">
          ...
          {canAct && (alert.status === 'NEW' || alert.status === 'DETECTED') && (
